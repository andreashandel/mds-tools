...
Using this template, create a new repository called `yourname-portfolio` (include both first and last name, i.e., `FirstnameLastname-portfolio`). Choose a public repository, then create it.

Next, **clone** the repository to your local computer. We recommend organizing your local repository folders in some way that works well for you. Remember that GitHub repositories/folders should not be nested inside each other, and should not be synced with some other software (e.g., Dropbox/OneDrive).


## Quick local website check

Open the repository folder on your computer as a project. Then open the main `index.qmd` file. You should see the `Preview` button in the upper left corner of the editor. Click it to render **the website**. Alternatively, you can type `quarto render` into the terminal.
