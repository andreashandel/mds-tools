...
View the slide deck for this unit: [Slides](github-additional-topics-slides.qmd){target="_blank" rel="noopener"}
...
Read the text transcript: [Text transcript](github-additional-topics-transcript.md)
...
* Create a `.gitignore` file for one of your repositories. Add entries to exclude files or folders that should not be tracked (e.g., large data files, temporary files, or sensitive information).
* Intentionally create a merge conflict by editing the same file in two different locations (e.g., on GitHub.com and on your local machine). Then practice resolving the conflict.
