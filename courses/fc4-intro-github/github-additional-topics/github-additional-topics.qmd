...
To resolve a merge conflict, you need to manually edit the conflicting file(s) to decide which changes to keep. Git will mark the conflicting sections in the file with special markers.
...
GitHub is not good at handling large files. If you try to push a file larger than 100 MB, GitHub will reject the push. Even files smaller than that can cause performance issues if they are large (e.g., several tens of MBs). If you need to work with large files, there are a few options. 
