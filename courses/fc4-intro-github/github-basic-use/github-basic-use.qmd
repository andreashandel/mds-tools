...
Let's give this whole workflow a try.  We will start by creating a repository online. Go to GitHub.com, make sure you are logged into your account and on your main screen. The URL you should be one looks something like `https://github.com/githubusername`. Click on the `Repositories` tab. Then create a new repository by clicking on the `New` button. Give the new repo, e.g., `my-test-repo`. Give it a short description, turn on 'add README', keep visibility to public, and keep the rest of the settings as they are. Then click **Create repository**. 
