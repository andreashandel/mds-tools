...
Theme selection is important to prevent eye fatigue or if you are color blind. Darker themes are generally better for your eyes during long coding sessions. There are a few themes that are specifically made for the color blind that rely on hue to differentiate code. Soloarized themes are generally the best for those who are colorblind. 

To be clear the themes do not change anything about the code it is for the humans writing/reading the code. They color and style token types (the different functional parts or key words) produced by the language parser (not arbitrary text). 
...
For instance, in Positron, to quickly turn code into comments (by adding a `#` in front of it for R cde), the default is to select the code chunk you want to comment out and then press the `Ctrl/Cmd + /` keyboard shortcut. This differs from RStudio, where it is the `Ctrl/Cmd + Shift + C` shortcut. If you prefer the RStudio shortcut, you can change it in Positron. 
