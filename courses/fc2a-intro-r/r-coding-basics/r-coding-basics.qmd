...
This whole course, including this unit, are by no means sufficient to
...
In the past, we recommended to start with Google, StackOverflow, and the
Internet to find relevant help. At this point, most of the time, you
will likely get the best initial help from some AI tool. We recommend to
start there, and if that doesn't solve your problem, then move on to
other sources.
...
3.  Someone has had the same problem before and written about it on the
    ...
    Search like you’d write a function call, not a sentence and should
    generally follow this pattern function name + argument names + error
    keyword. (i.e., “R reshape data error” is a bad search while “R
    tidyr pivot_longer names_to values_to error duplicate names” is a
    good search)
...
i.e. `2 -> x`. This is unususal coding style and best avoided. Just know
that it exists too.
...
We discussed some basics of writing R code. To re-iterate, this course
