...
<iframe class="video" src="https://www.youtube.com/embed/VIDEO_ID" title="AI tools for fixing code" allowfullscreen></iframe>
...
```{r ai-fix-code-r-quiz}
#| echo: false
#| results: "asis"
quizzes <- list("ai-fix-code-r-quiz-1.Rmd", "ai-fix-code-r-quiz-2.Rmd", "ai-fix-code-r-quiz-3.Rmd")
exams2forms::exams2forms(file = quizzes, title = 'ai-fix-code-r-quiz')
```
:::
<!-- end unit-quiz div -->
...
* Take a short R script that errors and ask an AI tool to fix it; compare with your manual fix.
* Share the exact error message with the AI and note how the response changes.
* Iterate once by asking for a specific improvement (e.g., better variable names or comments).
