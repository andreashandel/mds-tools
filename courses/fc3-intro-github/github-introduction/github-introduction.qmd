...
This unit provides a brief general introduction to Git and GitHub.
...
::: {.unit-goals}
# Goals
* Understand the reasons for using Git Explain the purpose of version control  
* Describe the difference between Git (tool) and GitHub (service)  
* Perform a basic Git workflow: add → commit → push → pull  
* Interpret commit history and resolve simple merge conflicts
:::
...
A **version‑control system (VCS)** records snapshots of your project
over time. Rather than duplicating folders like `manuscript_v3_FINAL`,
Git stores small *deltas* and metadata (author, timestamp, message).

### What is Git

[**Git**](https://git-scm.com/) is software that implements what is called a *version control system*. The main idea is that as you (and your collaborators) work on a project, the software tracks, and records any changes made by anyone. **Git** tracks every change to your files. It is a piece of software that does all the tracking.
...
[**GitHub**](https://github.com/) is distinct from Git. **GitHub** hosts changes you make and track with Git in the cloud. This makes collaboration and sharing easy. GitHub is basically a web platform where you can store your projects in the cloud.  

GitHub is in some sense on online interface/website, and Git the underlying engine (a bit like RStudio and R). 

You can use Git without using GitHub. Other interfaces/platforms for Git exist, e.g., [GitLab](https://about.gitlab.com/) or [Bitbucket](https://bitbucket.org/product). But the combination of Git and GitHub is the most common setup, and it is what we will be using for all of QSPL.
...
There are a number of reasons for using Git and GitHub, and there are other reasons why different systems might be better. The simple answer is that we are using Git/GitHub as part of QSPL (and beyond) since it is the most widely used tool for version control and thus a good default choice.
